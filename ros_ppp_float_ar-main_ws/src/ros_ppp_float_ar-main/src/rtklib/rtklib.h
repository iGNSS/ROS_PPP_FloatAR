...
/* add for code2freq API for rostopic /range_meas publish */
...
typedef struct {        /* receiver raw data control type */
    ...
    // HACK
    unsigned char nmeabuff[MAXRAWLEN]; /* message buffer */
    int           nmeasize; 

    unsigned char cstrParsedGPGGA[1024];
    unsigned char cstrParsedGPRMC[1024];
...
typedef struct {        /* observation data record */
    ...
    int iode;                     /* IODE */
    double clks;                   /* satellite time correction by SSR */
    double clkr;            /*receiver clock*/
    double orbXYZ[3];                   /* satellite position correction by SSR */
    double orbvXYZ[3];                   /* satellite position velocity correction by SSR */
    ...
    double ants[NFREQ ];            /* satellite antenna PCV */
...
EXPORT char *code2obs_in_code2freq(uint8_t code);
...
EXPORT int uraindex(int sys, double value);
