...
static int decode_ssr3(rtcm_t *rtcm, int sys)
{
    ...
    int i,j,k,type,mode,sync,iod,nsat,prn,sat,nbias,np,offp,ncode,cmode;
...
static int decode_ssr7(rtcm_t *rtcm, int sys)
{
    ...
    for (j=0;j<nsat&&i+5+17+np<=rtcm->len*8;j++) {
        ...
        for (k=0;k<nbias&&i+49<=rtcm->len*8;k++) {
            ...
            if (type<100) { /* experimental for MADOCA */
            ...
            /* trace(2,"decode_ssr_pbias: time=%s prn=%d mode=%d pbias=%f std=%f yaw_ang=%f sii=%d swl=%d sdc=%d\n",
            time_str(rtcm->time,2),prn,mode,bias,std,yaw_ang / 256.0*180.0,sii,swl,sdc);*/
        }
        if ((sys==SYS_BDS||sys==SYS_GLO)&&prn==0) prn=64;
        if (!(sat=satno(sys,prn))) {
            trace(2,"rtcm3 %d satellite number error: prn=%d\n",type,prn);
...
extern int decode_rtcm3(rtcm_t *rtcm)
{
    ...
    switch (type) {
        ...
        case 1265: ret=decode_ssr7(rtcm,SYS_GPS); break; /* tentative */       
        case 1267: ret=decode_ssr7(rtcm,SYS_GAL); break; /* tentative */
        case 1270: ret=decode_ssr7(rtcm,SYS_BDS); break; /* tentative */
