...
static unsigned int BDs_CRC(eph_t eph)
{
    ...
    BDSADDBITSFLOAT(11, eph.f2, 1.0 / (double)(1 << 30) / (double)(1 << 30) / (double)(1 << 6))
                      ...
                      int opt, double *rs, double *dts, double *var, int *svh)
{
    ...
    if (!ssr->t0[0].time)
    {
        trace(3, "no ssr orbit correction: %s sat=%2d\n", time_str(time, 0), sat);
        return 0;
    }
    if (!ssr->t0[1].time)
    {
        trace(3, "no ssr clock correction: %s sat=%2d\n", time_str(time, 0), sat);
        return 0;
    }
    ...
    if (ssr->udi[0] >= 1.0)
        t1 -= ssr->udi[0] / 2.0;
    if (ssr->udi[1] >= 1.0)
        t2 -= ssr->udi[1] / 2.0; /*changed from 0 to 1 by xiang 2020/12/14*/
