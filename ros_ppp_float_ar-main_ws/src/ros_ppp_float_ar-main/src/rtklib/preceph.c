...
static int readdcbf(const char *file, nav_t *nav, const sta_t *sta)
{
    FILE *fp;
    double cbias;
    static char buff[256], str1[32] = { 0 }, str2[32]={ 0 };
    int i,j,sat,type=0;
...
extern int read_snxbias(const char *file, nav_t *nav)
{
	...
	int sat, sec=-1, type = NULL, iscode = NULL, i, j,frq=0,codeIdx;
    biad_t *nav_bia;
    nav_bia = (obsd_t*)malloc(sizeof(biad_t) * MAXSAT);
    memset(nav_bia,0,sizeof(biad_t) * MAXSAT);

    nav->bia = nav_bia;
   /* memcpy(nav->bia, nav_bia, MAXSAT * sizeof(biad_t));*/
	...
	while (fgets(buff, sizeof(buff), fp)) {
		...
		if (sec == 1){
			...
			/*for GBM0MGXRAP BIA, new bia has SVN*/
