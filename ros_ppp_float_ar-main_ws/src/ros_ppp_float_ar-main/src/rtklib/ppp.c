...
#define MAX_STD_FIX 0.25            /* max std-dev (3d) to fix solution 0.15*/
                     ...
                     const double *rs, const double *rr, double *phw)
{
	...
	if (fabs(fabs(cosp)-1.0)<1.0e-12) return 0;
                     ...
                     const prcopt_t *opt)
{
	...
	/*correct the weight*/
	if (sinel>=0.5) return SQR(fact*opt->err[1])+SQR(fact*opt->err[2]*2);
	else return SQR(fact*opt->err[1])+SQR(fact*opt->err[2]/sinel);
 ...
 //   return SQR(fact*c)*(SQR(a)+SQR(b/sinel));
	/*PANDA 软件的随机模型确定方法*/
	//if (el*R2D>=30) return SQR(fact*c)*SQR(a);
	//else return SQR(fact*c)*SQR(a)/(4*SQR(sinel));
}
...
static double mwmeas(const obsd_t *obs, const nav_t *nav)
{
	...
	P[0]=obs->P[0];
	P[i]=obs->P[i];
	if (obs->code[0]==CODE_L1C) P[0]+=nav->cbias[obs->sat-1][1];/*C1->P1*/
	if (obs->code[i]==CODE_L2C) P[i]+=nav->cbias[obs->sat-1][2];/*C2->P2*/

    /* P1-P2 dcb correction (P1->Pc,P2->Pc) */
	 C1= SQR(lam[i])/(SQR(lam[i])-SQR(lam[0]));
     C2=-SQR(lam[0])/(SQR(lam[i])-SQR(lam[0]));
     P[0]-=C2*nav->cbias[obs->sat-1][0];
     P[i]+=C1*nav->cbias[obs->sat-1][0];

    return lam[0]*lam[i]*(obs->L[0]-obs->L[i])/(lam[i]-lam[0])-
           (lam[i]*P[0]+lam[0]*P[i])/(lam[i]+lam[0]);
}
                      ...
                      double *Lc, double *Pc)
{
    ...
    *Lc=*Pc=0.0;
    ...
    if (fabs(obs->P[0]-obs->P[i])>30) return;

	/* BDS2 code pseudorange multipath*/
	if (sys == SYS_CMP && P[0]!=0.0 && P[i]!=0.0) {
...
{
    double g0,g1,tt[MAXOBS],thres;
    int i,j;
    
    trace(3,"detslp_gf: n=%d\n",n);

    /*根据采样频率设置Threshold，参考GAMP*/
	for (i=0;i<n&&i<MAXOBS;i++){
		tt[i]=timediff(obs[i].time,rtk->ssat[obs[i].sat-1].pt[1][0]);
	}
	for (i=0;i<n-1;i++)for(j=i+1;j<n;j++){
		if(tt[i]<=tt[j]) continue;
		 SWAP_D(tt[i],tt[j]);
	}
	if (tt[0]<=1.0)        thres=rtk->opt.thresslip;
	else if (tt[0]<=20.0)  thres=(0.10)/(20.0-0.0)*tt[0]+rtk->opt.thresslip;
	else if (tt[0]<=60.0)  thres=3*rtk->opt.thresslip;
	else if (tt[0]<=100.0) thres=5*rtk->opt.thresslip;
	else                   thres=7*rtk->opt.thresslip;
...
static void detslp_mw(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
	...
	if (tt[0]<=1.0)        thres=2.5;// THRES_MW_JUMP=10
	else if (tt[0]<=20.0)  thres=(2.5)/(20.0-0.0)*tt[0]+2.5;
	else if (tt[0]<=60.0)  thres=5.0;
	else                   thres=7.0;
...
static void udclk_ppp(rtk_t *rtk)
{
    double dtr;
    int i;
	const int sys[]={SYS_GPS,SYS_GLO,SYS_GAL,SYS_CMP,SYS_QZS,SYS_IRN,SYS_LEO};
    ...
    for (i=0;i<NSYS;i++) {
		...
		if(i&&fabs(rtk->sol.dtr[i])<1.0e-16&&!(sys[i]&rtk->opt.navsys)) dtr=0;// dtr
        initx(rtk,CLIGHT*dtr,VAR_CLK,IC(i,&rtk->opt));
    }
/*--------------------------原始代码--------------------------*/
	//for (i=0;i<NSYS;i++) { 
	//		dtr=rtk->sol.dtr[i];
	//		if(fabs(dtr)<1.0e-16) dtr=0;
	//		initx(rtk,CLIGHT*dtr,VAR_CLK,IC(i,&rtk->opt));
	//}
}
...
static void udiono_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    for (i=0;i<n;i++) {
        ...
        if (rtk->x[j]==0.0) {
            ...
            rtk->P[j+j*rtk->nx]+=SQR(rtk->opt.prn[1]/sinel)*fabs(rtk->tt)+VAR_IONO;
        }
    }
}
    ...
    for (f=0;f<NF(&rtk->opt);f++) {
        ...
        for (i=k=0;i<n&&i<MAXOBS;i++) {
            ...
            if (rtk->opt.ionoopt==IONOOPT_IFLC) {
                ...
                if (fabs(bias[i]) >= 5000) slip[i] = 1;//data lost
            }
            else if (1/*L[f]!=0.0&&P[f]!=0.0*/) {
        ...
        for (i=0;i<n&&i<MAXOBS;i++) {
           ...
           // rtk->P[j+j*rtk->nx]+=SQR(rtk->opt.prn[0])*fabs(rtk->tt);
			rtk->P[j+j*rtk->nx]+=SQR(0.0)*fabs(rtk->tt);
...
/* tropospheric model ---------------------------------------------------------*/
static int ppprtk_trop(gtime_t time, const double *pos, const double *azel,
                      const prcopt_t *opt, const double *x, double *dtdx,
                      const nav_t *nav, double *dtrp, double *var)
{
    double trp[3]={0},std[3];
                   ...
                   double *azel)
{
    ...
    char str[32]; char s[64];/*为了输出电离层残差定义*/
    int ne=0,obsi[MAXOBS*2*NFREQ]={0},frqi[MAXOBS*2*NFREQ],maxobs,maxfrq,rej;
    int i,j,k,sat,sys,nv=0,nx=rtk->nx,stat=1,week=0;
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
		...
		//if(((int)(time2gpst(obs[i].time,&week))%90)==0)
		//{trace(2,"antmodel: dant=%10.3f %6.3f %6.3f\n",time2gpst(obs[i].time,&week),dantr[0],dantr[2]);}
        ...
        for (j=0;j<2*NF(opt);j++) {
            ...
            else {//这里需要修改，考虑GAL E5a在第三个存储的位置
                if(sys==1)
				{if ((y=j%2==0?L[j/2]:P[j/2])==0.0) continue;}
				else
				{if (j<=1) y=j%2==0?L[j/2]:P[j/2];
				else y=j%2==0?L[j/2+1]:P[j/2+1];
				if (y==0.0) continue;
				}         
                /* receiver DCB correction for P2 */
                if (j/2==1) dcb=-nav->rbias[0][sys==SYS_GLO?1:0][0];
            }
            if(sys==1)C=SQR(lam[j/2]/lam[0])*ionmapf(pos,azel+i*2)*(j%2==0?-1.0:1.0);
			else
			{ if(j<=1)C=SQR(lam[j/2]/lam[0])*ionmapf(pos,azel+i*2)*(j%2==0?-1.0:1.0);//修改 by Yin
			  else C=SQR(lam[2]/lam[0])*ionmapf(pos,azel+i*2)*(j%2==0?-1.0:1.0);}
...
/* phase and code residuals --------------------------------------------------*/
extern int ppprtk_res(const obsd_t *obs, int n, const nav_t *nav,rtk_t *rtk,aug_t *AUG)
{
    const double *lam;
    prcopt_t *opt=&rtk->opt;
    double y,r,cdtr,bias,C,rr[3],pos[3],e[3],dtdx[3],L[NFREQ],P[NFREQ],Lc,Pc;
	...
	double dr[3] = {0},P1_P2, gamma;

	double *rs, *dts, *varr, *azel,*svh,*x;

	rs=mat(6,n); dts=mat(2,n); varr=mat(1,n); azel=zeros(2,n);svh=mat(1,n);
	x=mat(rtk->nx,1);

	matcpy(x,rtk->xa,rtk->nx,1);
      
	/* satellite positions and clocks */
    satposs(obs[0].time,obs,n,nav,rtk->opt.sateph,rs,dts,varr,svh);

	/* earth tides correction */
    if (opt->tidecorr) {
     tidedisp(gpst2utc(obs[0].time),rtk->xa,opt->tidecorr==1?1:7,&nav->erp,
                 opt->odisp[0],dr);
    }
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        /* tropospheric and ionospheric model */
        if (!ppprtk_trop(obs[i].time,pos,azel+i*2,opt,x,dtdx,nav,&dtrp,&vart)) {
            continue;
        }
        /* satellite and receiver antenna model */
        if (opt->posopt[0]) satantpcv(rs+i*6,rr,nav->pcvs+sat-1,dants);
		...
		j = 1;//GPS
		if (sys&(SYS_GAL|SYS_SBS|SYS_CMP)) j=2;
		gamma=SQR(lam[j])/SQR(lam[0]);
		dcb = P1_P2 / (1.0 - gamma);
        ...
        for (j=0;j<2;j++) {
            ...
            bias=0.0;
            /* residual */
			bias =L[0]-(r+cdtr-CLIGHT*dts[i*2]+dtrp+lam[0]*AUG[sat-1].upd1+lam[0]*AUG[sat-1].N1);
			AUG[sat-1].res = -bias;
            nv++;
        }
    }
	free(rs); free(dts); free(varr); free(azel); free(svh); free(x);
    return nv;
}

/* phase and code residuals --------------------------------------------------*/
extern int gen_L4(const obsd_t *obs, int n, const nav_t *nav,rtk_t *rtk,aug_t *AUG)
{
    const double *lam;
    prcopt_t *opt=&rtk->opt;
    double y,r,cdtr,bias,C,rr[3],pos[3],e[3],dtdx[3],L[NFREQ],P[NFREQ],Lc,Pc;
...
{
    const prcopt_t *opt=&rtk->opt;
    double *rs,*dts,*var,*v,*H,*R,*azel,*xp,*Pp,dr[3]={0},std[3],dmp[3],disbds,dpos[3]={0.0};
    char str[32];
    int i,j,nv,info,svh[MAXOBS],exc[MAXOBS]={0},stat=SOLQ_SINGLE;

	int sat,index=0,flag=0;double ep[6]={0.0},LC,MW,QQ,secofday,lam_mw;
	const double *L_WAVE;//输出计算UPD的中间文件
    ...
    for (i=0;i<MAX_ITER;i++) {/*MAX_ITER*/
        ...
        if ((info=filter(xp,Pp,H,v,R,rtk->nx,nv))) {
            trace(3,"%s ppp (%d) filter error info=%d\n",str,i+1,info);
            break;
        }
        /* postfit residuals */
	...
	// 输出用于UPD估计的中间文件
 //   for (i=0;i<n&&i<MAXOBS;i++) {
	//sat=obs[i].sat;time2epoch(rtk->sol.time,ep);
	//  if (rtk->ssat[sat-1].vsat[0]==1&&satsys(sat,NULL)==32) {
	//	flag++;
	//	index=IB(sat,0,opt);
	//	LC=rtk->x[index];
	//	L_WAVE=nav->lam[sat-1];// Lam for L1/L2/L5
	//	if(satsys(sat,NULL)==32||satsys(sat,NULL)==8)lam_mw=L_WAVE[0]*L_WAVE[2]/(L_WAVE[2]-L_WAVE[0]);
	//	else lam_mw=L_WAVE[0]*L_WAVE[1]/(L_WAVE[1]-L_WAVE[0]);// GPS

	//	/* BDS2 */
	//	if(satsys(sat,NULL)==32) {
	//		disbds=0.0;
	//		BDmultipath(obs+i, rtk->ssat[sat-1].azel[1],dmp);
	//		disbds=(L_WAVE[2]*dmp[0]+L_WAVE[0]*dmp[2])/(L_WAVE[2]+ L_WAVE[0]);
	//		MW=(rtk->ssat[sat-1].mw-disbds)/lam_mw;//
	//	}
	//	else{
	//		MW=rtk->ssat[sat-1].mw/lam_mw;
	//	}
	//	QQ=rtk->P[index+index*rtk->nx];
	//	secofday=ep[5]+ep[4]*60+ep[3]*3600;
 //       if(satsys(sat,NULL)==SYS_CMP)trace(2,"%-6.0f%-3dAC %-3dMW %-13.5fLC %-13.5fnQ %-3dQ %9.8f\n",secofday,sat-114,rtk->ssat[sat-1].slipc[0],MW,LC,flag,QQ);
 //       else trace(2,"%-6.0f%-3dAC %-3dMW %-13.5fLC %-13.5fnQ %-3dQ %9.8f\n",secofday,sat,rtk->ssat[sat-1].slipc[0],MW,LC,flag,QQ);
 //           //if(sat==120) trace(2,"%-6.0f%-3d%-13.5f%-6.3f\n",secofday,sat,MW, rtk->ssat[119].azel[1]*R2D);
	//   }
 //   }
		...
		/*ambiguity resolution in ppp*/
			if (opt->modear==ARMODE_PPPAR || opt->modear==ARMODE_PPPAR_ILS)
			{
				if (pppamb(rtk,obs,n,nav,azel,exc)) {
     				...
     				matcpy(rtk->x, xp, rtk->nx, 1); // 传递浮点协方差阵
					matcpy(rtk->P,Pp,rtk->nx,rtk->nx);

					for (i=0;i<3;i++) std[i] = sqrt(rtk->Pa[i+i*rtk->nx]);
					if (norm(std,3)<MAX_STD_FIX) stat=SOLQ_FIX;

                    for (i=0;i<3;i++) dpos[i]=rtk->xa[i]-rtk->x[i];
                    if (norm(dpos,3)>0.5) stat=SOLQ_PPP;
				}
			}
        }
        else {
