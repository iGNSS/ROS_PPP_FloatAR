...
extern void output_osr_txt(FILE *fp, const obsd_t *obs, const osrd_t *osr, const int n, rtk_t *rtk)
{
    int i, k, sys, sat, prn;
    double tow, pos[3];
    ssat_t *ssat;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (fp)
        {
            fprintf(fp, "OSRRES,%.1f,%d,%d,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,"
                    ...
                    osr[k].orbXYZ[0], osr[k].orbXYZ[1], osr[k].orbXYZ[2],
                    osr[k].orbvXYZ[0], osr[k].orbvXYZ[1], osr[k].orbvXYZ[2],
                    osr[k].clks, osr[k].clkr,
                    osr[k].c[0], osr[k].c[1], osr[k].c[2],
                    osr[k].p[0], osr[k].p[1], osr[k].p[2],
...
static void write_osr(const obsd_t *obs, nav_t *nav, const osrd_t *osr,
                      const rtk_t *rtk, const int n, prcopt_t *opt)
{
    ...
    ecef2pos(rr, pos);
    /*output_osr_txt(fp_osr, obs, osr, n, rtk);*/
...
{
    ssat_t *ssat;
    double tow, pos[3] = {0}, posp[3] = {0}, vel[3], acc[3], *x;
    int i, j, k, week;
    char id[32], *p = buff;
        ...
        for (i = 0; i < MAXSAT; i++)
        {
            ...
            if (ssat->vsat[0] && ssat->vs)
            {
                ...
                if (rtk->x[j] == 0.0)
                    continue;
                satno2id(i + 1, id);
                double fs = ionppp(pos, rtk->ssat[i].azel, 6371, 300, posp);
                char info[256];
                sprintf(info, "$ION,%d,%.3f,%d,%s,%.1f,%.1f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.1f,%.1f,%.1f,%.1f\n", week, tow,
                        rtk->sol.stat, id, rtk->ssat[i].azel[0] * R2D,
                        rtk->ssat[i].azel[1] * R2D, x[j], STD(rtk, j), rtk->ssat[i].dion, rtk->ssat[i].vari, rtk->ssat[i].ion_brdc, posp[0], posp[1], fs,
                        rtk->ssat[i].threObs[0], rtk->ssat[i].threObs[1], rtk->ssat[i].threObs[2], rtk->ssat[i].threObs[3]);
                p += sprintf(p, info);
...
static void corr_bds2_multipath(obsd_t *obs, double *azel)
{
    ...
    if (!strcmp(BDsType[prn - 1], "BDS2-I"))
        nType = 1;
    else if (!strcmp(BDsType[prn - 1], "BDS2-M"))
        nType = 2;
    else if (!strcmp(BDsType[prn - 1], "BDS2-G"))
        nType = 2;
    else
        return;
    /* L2, L7, L6*/
    ...
    {
        for (i = 0; i < 3; i++)
            obs->P[i] += coef[0][(nType - 1) * 3 + i];
    }
    else if (azel[1] * R2D >= 90.0)
    {
        for (i = 0; i < 3; i++)
            obs->P[i] += coef[9][(nType - 1) * 3 + i];
    }
    else
    {
                      ...
                      double *Lc, double *Pc)
{
    ...
    *Lc = *Pc = 0.0;
    ...
    if (sys & SYS_BDS)
    {
        ...
        if (P[2] != 0)
            P[2] += (C2 / C3 * DCB12 - DCB15); /*(C2 *DCB12 - DCB15);*/
        if (P[0] != 0)
            P[0] += (C2 * DCB12);
        if (P[1] != 0)
            P[1] += (C1 * DCB12);
...
#endif
#if 0
    if (opt->ionoopt == IONOOPT_EST && nav->cbias) {	/* P1-P2 dcb correction (P1->Pc,P2->Pc) C1=gamma2/(1-gamma2) C2=1/(1-gamma2)*/
        if (sys & (SYS_GPS | SYS_GLO | SYS_QZS)) {
            if (P[0] != 0.0) P[0] += C2 * nav->cbias[obs->sat - 1][0];
            if (P[1] != 0.0) P[1] += C1 * nav->cbias[obs->sat - 1][0];
        }
    ...
    static void corr_meas_bia(const obsd_t *obs, const nav_t *nav, const double *azel,
                              const prcopt_t *opt, const double *dantr,
                              const double *dants, double phw, double *L, double *P,
                              double *Lc, double *Pc, osrd_t *osr)
    {
        const double *lam = nav->lam[obs->sat - 1];
        double C1, C2, C3, DCB12, DCB15, DCB13;
        int i, sys, ix = 0;
        osrd_t *osrtmp;
        osrtmp = (osrd_t *)malloc(sizeof(osrd_t));
        osr = osrtmp;
        ...
        for (i = 0; i < NFREQ; i++)
        {
            ...
            osr->wupL[i] = phw * lam[i];
            osr->antr[i] = dantr[i];
            osr->ants[i] = dants[i];
            osr->c[i] = obs->L[i] * lam[i];
            osr->p[i] = obs->P[i];
            if (opt->ionoopt == IONOOPT_EST && nav->bia)
            {
                if (opt->sateph == EPHOPT_SSRAPC || opt->sateph == EPHOPT_SSRCOM)
                ...
                else if (nav->bia)
                {
                    if (nav->bia[obs->sat - 1].cBias[obs->code[i] - 1] != 0)
                    {
                        ...
                        osr->cbias[i] = nav->bia[obs->sat - 1].cBias[obs->code[i] - 1];
                        osr->pbias[i] = nav->bia[obs->sat - 1].lBias[obs->code[i] - 1];
                    }
                }
                else
                {
        ...
        *Lc = *Pc = 0.0;
        ...
        /*choose pseudorange measurements BDS GALILEO AND SBAS are based on B3/E5A and align to P obs*/
        if (opt->ionoopt == IONOOPT_EST)
        {
            /*if (sys & (SYS_GPS | SYS_GLO | SYS_QZS)) {
                if (P[0] != 0.0 && !nav->bia) P[0] += C2 * nav->cbias[obs->sat - 1][0];
                if (P[1] != 0.0 && !nav->bia) P[1] += C1 * nav->cbias[obs->sat - 1][0];
            }*/
                           ...
                           const nav_t *nav)
    {
        ...
        for (i = 0; i < n && i < MAXOBS; i++)
        {
            ...
            for (f = 0; f < rtk->opt.nf; f++)
            {
                ...
                if (fabs(tt = timediff(obs[i].time, rtk->ssat[sat - 1].pt[0][f])) < DTTOL)
                    continue;
                ...
                rtk->ssat[sat - 1].ddop[f] = fabs(dph - dpt);
    ...
    static void detslp_gf(rtk_t * rtk, const obsd_t *obs, int n, const nav_t *nav)
    {
        ...
        for (i = 0; i < n && i < MAXOBS; i++)
        {
            ...
            if (g0 != 0.0 && fabs(g1 - g0) > rtk->opt.thresslip)
            {
                ...
                trace(4, "detslp_gf: slip detected %s sat=%3d %3s el=%8.3f SNR=%5.1f %5.1f gf=%8.3f->%8.3f diff = %8.3f\n",
    ...
    static void detslp_mw(rtk_t * rtk, const obsd_t *obs, int n, const nav_t *nav)
    {
        ...
        for (i = 0; i < n && i < MAXOBS; i++)
        {
            ...
            if (rtk->ssat[obs[i].sat - 1].mwarc >= 4)
            {
                ...
                {
                    satno2id(obs[i].sat, id);
                    trace(4, "detslp_mw: slip detected %s sat=%3d %3s el=%8.3f SNR=%5.1f %5.1f mw=%8.3f->%8.3f\n",
    ...
    static void udbias_ppp(rtk_t * rtk, const obsd_t *obs, int n, const nav_t *nav)
    {
        ...
        double ion = 0, dantr[NFREQ] = {0}, dants[NFREQ] = {0}, offseti[MAXOBS] = {0};
        ...
        for (f = 0; f < NF(&rtk->opt); f++)
        {
            ...
            for (i = k = 0; i < n && i < MAXOBS; i++)
            {
                sat = obs[i].sat;
                j = IB(sat, f, &rtk->opt);
                corr_meas_bia(obs + i, nav, rtk->ssat[sat - 1].azel, &rtk->opt, dantr, dants,
                              0.0, L, P, &Lc, &Pc, osr);
                ...
                else if (L[f] != 0.0 && P[f] != 0.0)
                {
                    ...
                    if (fabs(obs[i].P[0] - obs[i].P[l]) < 30.0)
                          ...
                          const nav_t *nav, double *dion, double *var)
    {
        ...
        if (opt->ionoopt == IONOOPT_STEC)
        {
            if (timediff(time, time_p) != 0.0 &&
                ...
                return 0;
            if (iono_p[sat - 1] == 0.0 || std_p[sat - 1] > 0.1)
                return 0;
            time_p = time;
            *dion = iono_p[sat - 1];
            *var = SQR(std_p[sat - 1]);
                          ...
                          double *var)
    {
            ...
            pppcorr_stec(&nav->pppcorr, time, pos, iono, std_iono))
        {
            ...
            {
                sat = obs[i].sat;
                if (exc[i] || iono[sat - 1] == 0.0 || std_iono[sat - 1] > 0.5)
                    continue;
                j = II(sat, &rtk->opt);
                v[nv] = iono[sat - 1] - x[j];
                for (k = 0; k < rtk->nx; k++)
        ...
        /*update the dion and vari using gim model*/
        if (rtk->opt.ionoopt == IONOOPT_EST)
        {
            for (i = 0; i < n; i++)
            {
                sat = obs[i].sat;
                iontec(time, nav, pos, azel + 2 * i, 1, iono, std_iono);
                double ion_brdc = ionmodel(obs[0].time, nav->ion_gps, pos, azel + 2 * i);
                if (!iono[0])
                    continue;
                rtk->ssat[sat - 1].dion = iono[0];
                rtk->ssat[sat - 1].vari = SQRT(std_iono[0]);
                rtk->ssat[sat - 1].ion_brdc = ion_brdc;
            }
        }
        return nv;
    }
                       ...
                       double *azel, osrd_t *osr)
    {
        ...
        for (i = 0; i < n && i < MAXOBS; i++)
        {
            ...
            satno2id(sat, id);
            lam = nav->lam[sat - 1];
            if (lam[j / 2 - 1] == 0.0 || lam[0] == 0.0)
                continue;
            osr[i].sat = sat;
            osr[i].orbXYZ[0] = *(rs + i * 6);
            osr[i].orbXYZ[1] = *(rs + i * 6 + 1);
            osr[i].orbXYZ[2] = *(rs + i * 6 + 2);
            osr[i].orbvXYZ[0] = *(rs + i * 6 + 3);
            osr[i].orbvXYZ[1] = *(rs + i * 6 + 4);
            osr[i].orbvXYZ[2] = *(rs + i * 6 + 5);
            osr[i].clks = CLIGHT * dts[i * 2];
            ...
            /*added by xiang to debug dion and vari for constraints*/
            ...
            if (sys & SYS_BDS)
                corr_bds2_multipath(obs + i, azel + i * 2);

            /* corrected phase and code measurements */
            corr_meas_bia(obs + i, nav, azel + i * 2, &rtk->opt, dantr, dants,
                          rtk->ssat[sat - 1].phw, L, P, &Lc, &Pc, osr + i);

            /* stack phase and code residuals {L1,P1,L2,P2,...} */
            for (j = 0; j < 2 * NF(opt); j++)
            {
                /*            if (opt->ionoopt == IONOOPT_EST && sys == (SYS_GAL | SYS_SBS | SYS_BDS)) {
                                if (j == 2 || j == 3) j = j + 2;
                            }*/
                /*only B1 and B3 are needed*/
                ...
                else
                {
                    ...
                    if (j / 2 == 1)
                        dcb = -nav->rbias[0][sys == SYS_GLO ? 1 : 0][0];
                }
                ...
                if (sys & (SYS_GAL | SYS_SBS | SYS_BDS))
                {
                    if (j == 2 || j == 3)
                    {
                        C = SQR(lam[(j + 2) / 2] / lam[0]) * (j % 2 == 0 ? -1.0 : 1.0);
                    }
                }
        ...
        /*}*/
                ...
                rtk->ssat[sat - 1].threObs[j] = v[nv] / sqrt(var[nv]);
    ...
    extern void pppos(rtk_t * rtk, const obsd_t *obs, int n, const nav_t *nav)
    {
        ...
        xLast = mat(rtk->nx, 1);
        PLast = zeros(rtk->nx, rtk->nx);
        xGFm = mat(rtk->nx, 1);
        PGFm = zeros(rtk->nx, rtk->nx);
        ...
        for (i = 0; i < MAX_ITER; i++)
        {
            ...
            /* prefit residuals */
            if (!(nv = ppp_res(0, obs, n, rs, dts, var, svh, dr, exc, nav, xp, rtk, v, H, R, azel, osr)))
            {
            ...
            /* postfit residuals */
            if (ppp_res(i + 1, obs, n, rs, dts, var, svh, dr, exc, nav, xp, rtk, v, H, R, azel, osr))
            {
        ...
        if (stat == SOLQ_PPP)
        {
            ...
            if (ppp_ar(rtk, obs, n, exc, nav, azel, xp, Pp) &&
                ppp_res(9, obs, n, rs, dts, var, svh, dr, exc, nav, xp, rtk, v, H, R, azel, osr))
            {
        ...
        write_osr(obs, nav, osr, rtk, n, &rtk->opt);
