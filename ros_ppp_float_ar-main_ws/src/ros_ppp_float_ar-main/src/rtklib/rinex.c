...
extern int uraindex(int sys,double value)
{
    int i;
    if (sys==SYS_GAL) {
        ...
        if (strlen(buff)<=60) continue;
            ...
            *type=*(buff+20);
            ...
            switch (*(buff+40)) {
                ...
                case 'C': *sys=SYS_BDS;  *tsys=TSYS_BDS; break; /* v.2.12 */
...
extern int outrnxobsh(FILE *fp, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    if (opt->rnxver<=2.99) { /* ver.2 */
        ...
        else if (opt->navsys==SYS_BDS) sys="C: BeiDou"; /* ver.3.02 */
...
extern int outrnxnavh(FILE *fp, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    if (opt->rnxver<=2.99) { /* ver.2 */
        ...
        else if (opt->navsys==SYS_BDS) sys="C: BeiDou"; /* v.3.02 */
    ...
    if (opt->rnxver<=2.99) { /* ver.2 */
        ...
        if ((opt->navsys&SYS_BDS)&&opt->rnxver>=3.02) { /* ver.3.02 */
            if (opt->outtime) {
                fprintf(fp,"BDUT %17.10E%16.9E%7.0f%5.0f %-5s %-2s %-20s\n",
                        nav->utc_bds[0],nav->utc_bds[1],nav->utc_bds[2],
                        nav->utc_bds[3],"","","TIME SYSTEM CORR");
            }
        }
        if ((opt->navsys&SYS_IRN)&&opt->rnxver>=3.03) { /* ver.3.03 */
            if (opt->outtime) {
...
extern int outrnxnavb(FILE *fp, const rnxopt_t *opt, const eph_t *eph)
{
    ...
    if (opt->rnxver>2.99||sys==SYS_GAL||sys==SYS_BDS) { /* ver.3 or ver.2 GAL */
...
extern int outrnxhnavb(FILE *fp, const rnxopt_t *opt, const seph_t *seph)
{
    ...
    if ((sys=satsys(seph->sat,&prn)&opt->navsys)!=SYS_SBS) return 0;
    ...
    outnavf(fp,uravalue(sys,seph->sva));
    fprintf(fp,"\n%s",sep         );
