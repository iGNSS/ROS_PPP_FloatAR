...
static int decode_ssr1_head(rtcm_t *rtcm, int sys, int cmode, int *sync, int *iod,
                            double *udint, int *refd, int *hsize)
{
    ...
    *udint=ssrudint[udi];
	/*if (sys == SYS_BDS)/*add to calculate IOD*/
	/*{
		*iod= ((int)tow / 720) % 240;
	}*/
    time2str(rtcm->time,tstr,2);
...
static int decode_ssr2_head(rtcm_t *rtcm, int sys, int cmode, int *sync, int *iod,
...
static int decode_ssr1(rtcm_t *rtcm, int sys)
{
    ...
    int i,j,k,type,sync,iod,nsat,prn,sat,iode,iodcrc,refd=0,np,ni,nj,offp,cmode;
    
    type=getbitu(rtcm->buff,24,12);
    cmode = (type==4076)?IGS_SSR:RTCM_SSR;
    
    if ((nsat=decode_ssr1_head(rtcm,sys,cmode,&sync,&iod,&udint,&refd,&i))<0) {
        trace(2,"rtcm3 %d length error: len=%d\n",type,rtcm->len);
        return -1;
    }
    switch (sys) {
        ...
        /*case SYS_BDS: np=6; ni=10; nj=24; offp=  0; break;*/
    ...
    for (j=0;j<nsat&&i+121+np+ni+nj<=rtcm->len*8;j++) {
        ...
        if (sys == SYS_BDS) {
            ...
            i += 8;/*change to 8 for huace*/
            iode =getbitu(rtcm->buff,i,nj);
            i += 10; /*change to 10 for huace*/
        } else {
        ...
        if ((sys==SYS_BDS||sys==SYS_GLO)&&prn==0) prn=64;
        if (!(sat=satno(sys,prn))) {
...
{
    double udint,dclk[3];
    int i,j,k,type,sync,iod,nsat,prn,sat,np,offp,cmode;
    
    type=getbitu(rtcm->buff,24,12);
    cmode = (type==4076)?IGS_SSR:RTCM_SSR;
    
    if ((nsat=decode_ssr2_head(rtcm,sys,cmode,&sync,&iod,&udint,&i))<0) {
        trace(2,"rtcm3 %d length error: len=%d\n",type,rtcm->len);
        return -1;
    }
    switch (sys) {
...
static int decode_ssr3(rtcm_t *rtcm, int sys)
{
    ...
    int i,j,k,type,mode,sync,iod,nsat,prn,sat,nbias,np,offp,cmode;/*delete ncode*/
    ...
    if (cmode==IGS_SSR) {
        switch (sys) {
            ...
            case SYS_BDS: offp=  0; /*codes=codes_bds_igs; ncode=19;*/ break;
        ...
        switch (sys) {
            ...
            case SYS_BDS: np=6; offp=  0; /*codes=codes_bds_rtcm; ncode=15;*/ break;
    ...
    for (j=0;j<nsat&&i+5+np<=rtcm->len*8;j++) {
        ...
        /*trace(2,"decode %d cbias: time=%s sat=%d nbias=%d\n",type,time_str(rtcm->time,2),satno(sys,prn),nbias);*/
        for (k=0;k<MAXCODE;k++) cbias[k]=0.0;
        for (k=0;k<nbias&&i+19<=rtcm->len*8;k++) {
            mode=getbitu(rtcm->buff,i, 6);      i+= 6;/* changed to 6 for huace */
            bias=getbits(rtcm->buff,i,14)*0.01; i+=14;
            if ((mode<=MAXCODE)&&(mode>=0)) {
                cbias[mode-1]=(float)bias;
            }
            else {
                trace(2,"rtcm3 %d not supported code: mode=%d\n",type,mode);
            }
            /*trace(2,"decode_ssr_cbias: time=%s sys=%d nbias=%d prn=%d mode=%d cbias=%f \n",time_str(rtcm->time,2),sys,nbias,prn,mode,bias*0.0001);*/
            /*trace(2,"mode=%d cbias=%f \n",mode,bias*0.0001);*/
        }
...
static int decode_ssr4(rtcm_t *rtcm, int sys)
{
    ...
    int i,j,k,type,nsat,sync,iod,prn,sat,iode,iodcrc,refd=0,np,ni,nj,offp,cmode;
    ...
    for (j=0;j<nsat&&i+191+np+ni+nj<=rtcm->len*8;j++) {
		...
		/*if (sys == SYS_BDS &&!rtcm->ssr[sat - 1].iode) rtcm->ssr[sat - 1].iode = iodcrc;*/
...
{
    double udint;
    int i,j,type,nsat,sync,iod,prn,sat,ura,np,offp,cmode;
...
{
    double udint,hrclk;
    int i,j,type,nsat,sync,iod,prn,sat,np,offp,cmode;
...
static int decode_ssr7_head(rtcm_t *rtcm, int sys, int cmode, int *sync, int *iod,
                            double *udint, int *dispe, int *mw, int *hsize)
{
    ...
    *udint=ssrudint[udi];
    ...
    time2str(rtcm->time,tstr,2);
    /*trace(2,"decode_ssr7_head: time=%s sys=%d nsat=%d sync=%d iod=%d provid=%d solid=%d udint=%f\n",
          tstr,sys,nsat,*sync,*iod,provid,solid,udi);*/
...
static int decode_ssr7(rtcm_t *rtcm, int sys)
{
    ...
    int i,j,k,type,mode,sync,iod,nsat,prn,sat,nbias,np,mw,offp,swl;/*delete sii,ncode*/
    int dispe,sdc,yaw_ang,yaw_rate,cmode,len;
    
    type=getbitu(rtcm->buff,24,12);
    cmode = (type==4076)?IGS_SSR:RTCM_SSR;
    len = (type<100)?49:32;
    
    if ((nsat=decode_ssr7_head(rtcm,sys,cmode,&sync,&iod,&udint,&dispe,&mw,&i))<0) {
        trace(2,"rtcm3 %d length error: len=%d\n",type,rtcm->len);
        return -1;
    }
    if (cmode==IGS_SSR) {
    ...
    for (j=0;j<nsat&&i+5+17+np<=rtcm->len*8;j++) {
        ...
        /*trace(2,"decode %d pbias: time=%s sat=%d nbias=%d\n",type,time_str(rtcm->time,2),satno(sys,prn),nbias);*/
        ...
        for (k=0;k<nbias&&i+49<=rtcm->len*8;k++) {
            ...
            if ((mode<=MAXCODE)&&(mode>=0)) {
                pbias[mode-1]=bias*0.0001; /* (m) */
                stdpb[mode-1]=std *0.0001; /* (m) */
            }
            else {
                trace(2,"rtcm3 %d not supported mode: mode=%d\n",type,mode);
            }
            /*trace(2,"decode_ssr_pbias: time=%s prn=%d nbias=%d mode=%d pbias=%f std=%f yaw_ang=%f swl=%d sdc=%d\n",time_str(rtcm->time,2),prn,nbias,mode,bias*0.0001,std,yaw_ang / 256.0*180.0,swl,sdc);*/
            /*trace(2,"mode=%d pbias=%f\n",mode,bias*0.0001);*/
            /*printf("mode=%d pbias=%f\n",mode,bias*0.0001);*/
...
/* decode ssr vtec message header ---------------------------------------------*/
static int decode_ssr_vtec_head(rtcm_t *rtcm, int cmode, int *sync, int *iod,
                            float *quality, double *udint, int *hsize)
{
    double tow;
    char *msg;
    int i=24+12,nlayer,udi,provid=0,solid=0;
    ...
    *quality = getbitu(rtcm->buff,i, 9)*0.05; i+= 9;
    nlayer = getbitu(rtcm->buff,i, 2)+1; i+= 2;

    *udint=ssrudint[udi];

    trace(4,"decode_ssr_vtec: time=%s nlayer=%d sync=%d iod=%d provid=%d solid=%d\n",
          time_str(rtcm->time,2),nlayer,*sync,*iod,provid,solid);

    if (rtcm->outtype) {
        msg=rtcm->msgtype+strlen(rtcm->msgtype);
        sprintf(msg," %s nlayer=%2d iod=%2d udi=%2d sync=%d",
                time_str(rtcm->time,2),nlayer,*iod,udi,*sync);
    }
    *hsize=i;
    return nlayer;
}
/* decode ssr vtec  [1264] ---------------------------------------*/
static int decode_ssrvtec(rtcm_t *rtcm)
{
    double udint;
    float quality;
    int i,j,n,m,type,sync,iod,nlayer,degree,order,cmode;
    vtec_t *vtec = &rtcm->vtec;

    type=getbitu(rtcm->buff,24,12);
    cmode = (type==4076)?IGS_SSR:RTCM_SSR;

    if ((nlayer=decode_ssr_vtec_head(rtcm,cmode,&sync,&iod,&quality,&udint,&i))<0) {
        trace(2,"rtcm3 %d length error: len=%d\n",type,rtcm->len);
        return -1;
    }
    ...
    vtec->udi = udint;
    vtec->iod = iod;
    vtec->quality = quality; /* 1-4 */
    vtec->nlayer = nlayer;

    for (j=0;j<nlayer;j++) {
        vtec->height[j] = getbits(rtcm->buff,i,8)*1E4; i+=8;
        degree = getbitu(rtcm->buff,i,4)+1; i+= 4;
        order = getbitu(rtcm->buff,i,4)+1; i+= 4;

        vtec->degree[j] = degree; /* 1-16:N */
        vtec->order[j]  = order;  /* 1-16:M */

        /* degree:N, order:M */
        m = (degree+1)*(degree+2)/2 - (degree-order)*(degree-order+1)/2;
        for (n=0;n<m;n++) {
            vtec->c[j][n] = getbits(rtcm->buff,i,16)*5E-3; i+=16;
        }
        m -= degree+1;
        for (n=0;n<m;n++) {
            vtec->s[j][n] = getbits(rtcm->buff,i,16)*5E-3; i+=16;
        }
    }
    vtec->update = 1;
...
static int decode_type4076(rtcm_t *rtcm)
{
    ...
    switch (subtype) {
        ...
        case 101: return decode_ssr1(rtcm, SYS_BDS);
        case 102: return decode_ssr2(rtcm, SYS_BDS);
        case 103: return decode_ssr4(rtcm, SYS_BDS);
        case 104: return decode_ssr6(rtcm, SYS_BDS);
        case 105: return decode_ssr3(rtcm, SYS_BDS);
        case 106: return decode_ssr7(rtcm, SYS_BDS);
        case 107: return decode_ssr5(rtcm, SYS_BDS);
                         ...
                         const int *ex, const int *half)
{
    ...
    for (i=0;i<h->nsig;i++) {
		...
		if (sys == SYS_BDS) {
            if (freq[i] == 1) freq[i] = 11; /* set C1P to a large value */
            else if (freq[i] == 3) freq[i] = 12; /* C5P to a large value */
            else if (freq[i] == 2) freq[i] = 1; /* 2I 2Q to B1I */
...
extern int decode_rtcm3(rtcm_t *rtcm)
{
    ...
    switch (type) {
        ...
        case 2080: ret=decode_ssr3(rtcm,SYS_GPS); break; /*1059 changed to 2080 for huace*/
        ...
        case 2081: ret=decode_ssr3(rtcm,SYS_GLO); break; /*1065 changed to 2081 for huace*/
        ...
        case 2082: ret=decode_ssr3(rtcm,SYS_GAL); break; /*1242 changed to 2082 for huace*/
        ...
        case 2084: ret=decode_ssr3(rtcm,SYS_BDS); break; /*1260 changed to 2084 for huace*/
        ...
        case 2065: ret=decode_ssr7(rtcm,SYS_GPS); break; /* tentative 1265 changed to 2065 for huace*/       
        case 2067: ret=decode_ssr7(rtcm,SYS_GAL); break; /* tentative 1267 changed to 2067 for huace*/
        case 2070: ret=decode_ssr7(rtcm,SYS_BDS); break; /* tentative 1270 changed to 2070 for huace*/
