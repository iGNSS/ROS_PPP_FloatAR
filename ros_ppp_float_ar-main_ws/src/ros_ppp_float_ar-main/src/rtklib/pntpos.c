...
/*------------------------------------------------------------------------------------*/
		/* doppler residual，不加任何改正的时候 */
        /*v[nv]=-lam*obs[i].D[0]-(rate+x[3]-CLIGHT*dts[1+i*2]);*/

/*------------------------------------------------------------------------------------*/
		/*考虑级数展开的二次项影响的办法*/
		Vs=0.5*(SQR(rs[3+i*6])+SQR(rs[4+i*6])+SQR(rs[5+i*6]))/CLIGHT;/*卫星速度V^2*/
...
/*------------------------------------------------------------------------------------*/
		/*考虑非圆轨道改正影响的办法，这个办法不是很好，直接在钟差改正比较好*/
...
/*------------------------------------------------------------------------------------*/
    /* estimate receiver velocity with doppler，原始的利用原始多普勒观测值求解的程序 */
    //if (stat) estvel(obs,n,rs,dts,nav,&opt_,sol,azel_,vsat);

   /*estimate receiver velocity with differential carrier phase,载波相位差分法测速*/
	satpossDopp(sol->time,obs,n,nav,opt_.sateph,rs,dts,var,svh);
	if (stat) estvel(obs,n,rs,dts,nav,&opt_,sol,azel_,svh,ssat);//修改vsat->svh
/*------------------------------------------------------------------------------------*/
