...
static void outsolstat(rtk_t *rtk, const nav_t *nav)
{
    ...
    for (i = 0; i < MAXSAT; i++)
    {
        ...
        {
            k = IB(i + 1, j, &rtk->opt);
            fprintf(fp_stat, "$SAT,%d,%.3f,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%.0f,%d,%d,%d,%d,%d,%d,%.2f,%.6f,%.5f,%.5f,%.4f,%.4f,%.4f,%.4f,%.4f,%d,%d,%d\n",
                    ...
                    ssat->mw, ssat->mwmean, ssat->gf, ssat->gfdiff, ssat->ddop[j],
                    ssat->slipLLI[j], ssat->slipMW[j], ssat->slipGF[j]); /*add mw, GF, slipGF;*/
        }
    }
}
/* save error message --------------------------------------------------------*/
                     ...
                     double bl, double dt, int f, const prcopt_t *opt, const obsd_t *obs)
{
    ...
    { /* normal error model */
        if (opt->rcvstds && obs->qualL[frq] != '\0' && obs->qualP[frq] != '\0')
        {
            ...
            if (code)
                fact = opt->eratio[frq] * obs->qualP[frq];
            else
                fact = obs->qualL[frq];
        }
        else if (code)
            fact = opt->eratio[frq]; /* use err ratio only */
        if (fact <= 0.0)
            fact = opt->eratio[0];
        switch (sys)
        {
    ...
    case WEIGHTOPT_ELEVATION:
        return fact * 2.0 * (SQR(a) + SQR(b / sinel) + SQR(c)) + SQR(d);
    case WEIGHTOPT_SNR:
        return fact * (SQR(a) *
        ...
        for (f = opt->mode > PMODE_DGPS ? 0 : nf; f < nf * 2; f++)
        {
            ...
            for (i = -1, j = 0; j < ns; j++)
            {
                ...
                if (rtk->ssat[sat[j] - 1].slip[f])
                    continue; /*without cycle slip added by xiang*/
            ...
            for (j = 0; j < ns; j++)
            {
                ...
                threshadj = code || (rtk->P[ii + rtk->nx * ii] > SQR(rtk->opt.std[0] / 2)) ||
                                    (rtk->P[jj + rtk->nx * jj] > SQR(rtk->opt.std[0] / 2))
                                ? opt->eratio[frq]
                                : 1;
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    if (opt->mode >= PMODE_PPP_KINEMA)
    {
        ...
        if ((rtk->P[0] + rtk->P[1] + rtk->P[2]) < 30 * 30)
            outsolstat(rtk, nav);
        else
            return 0;
