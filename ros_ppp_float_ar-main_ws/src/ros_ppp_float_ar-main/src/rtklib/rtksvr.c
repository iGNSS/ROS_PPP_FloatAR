...
void publish_glo_ephemeris(geph_t *geph);
void publish_corrections(ssr_t *ssr_data);
void publish_iono(rtksvr_t *svr);

void publish_nmea(rtksvr_t *svr);
...
#endif
{
    ...
    for (cycle=0;svr->state;cycle++) {
        ...
        // HACK
        // printf("CALL GPGGA: %s", svr->raw->cstrParsedGPGGA);
        // printf("CALL GPRMC: %s", svr->raw->cstrParsedGPGGA);
        publish_iono(svr);
        publish_nmea(svr);
