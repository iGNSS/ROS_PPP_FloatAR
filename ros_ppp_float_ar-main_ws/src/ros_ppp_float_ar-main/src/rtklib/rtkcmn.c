...
static char *obscodes[] = {       /* observation code strings revised by Lye */
...
/* 1:L1/E1/B1C, 2:L2/B1I, 3:L5/E5a/L3, 4:E6/LEX/B3/G2a, 5:B2I/E5b, 6:E5(a+b)/, 7:S  8:G1a */
   /*    0         1          2          3         4         5     */
    {"PCYWMNSL","PYWCMNDLSX","IQX"     ,""       ,""       ,""      ,""}, /* GPS */ /* GPS CPYWMNSL, PYWCMNDSLX*/
    {"CPABX"   ,"PCABX"     ,"IQX"     ,""       ,""       ,""      ,""}, /* GLO */
...
*-----------------------------------------------------------------------------*/
extern char *code2obs_in_code2freq(uint8_t code)
{
    if (code<=CODE_NONE||MAXCODE<code) return "";
    return obscodes[code];
}
...
{
    char *obs=code2obs_in_code2freq(code);
...
extern int code2idx(int sys, uint8_t code)
{
    ...
    switch (sys) {
        ...
        case SYS_BDS: return code2freq_BDS(code,&freq);
...
extern double code2freq(int sys, uint8_t code, int fcn)
{
    ...
    switch (sys) {
        ...
        case SYS_BDS: (void)code2freq_BDS(code,&freq); break;
...
/*extern double uravalue(int sva)//deleted by Yifan because of no Galileo
{
    return 0<=sva&&sva<15?ura_nominal[sva]:8192.0;
}*/
/* ura value (m) to ura index ------------------------------------------------*/
/*extern int uraindex(double value)//deleted by Yifan because of no Galileo
{
    int i;
...
extern void GPT(double *pos, gtime_t time, double *p, double *t)
{
	const double zero[55] = { 0 };
	...
	*						UNDU = -42.78796423912972813D0 meters*/
	...
	/*change data for debug*/
	/*mjd = 55055;
	pos[0] = 0.6708665767;
	pos[1] = -1.393397187;
	pos[2] = 812.546;*/

	double doy = mjd - 44239. - 27;/*since Jan 28, 1980*/

	double	dN = Legendre_Interpolation(a_geoid, zero, b_geoid, zero, 9, 9, doy, pos);
	double	dOrthometricHeight = pos[2] - dN;

	double	dPressure0 = Legendre_Interpolation(aP_mean, aP_amp, bP_mean, bP_amp, 9, 9, doy, pos);
	*p = dPressure0*pow((1 - 0.0000226*dOrthometricHeight), (5.225));

	double	dTemperature0 = Legendre_Interpolation(aT_mean, aT_amp, bT_mean, bT_amp, 9, 9, doy, pos);
	*t = dTemperature0 - 0.0065*dOrthometricHeight;

}

/******************************************************
Function name:     Legendre_Interpolation

Purpose:           2D Legendre sphere-harmonics function with degree and order up to 9 for GMF hydrostatic and wet coefficient a,
based on the Fortran code at http://www.hg.tuwien.ac.at/~ecmwf1/gpt.f

Input:        A_mean[]:     Interpolation coefficients for mean value
A_amp[]:      Interpolation coefficients for amplitude
B_mean[]:     Interpolation coefficients for mean value
B_amp[]:      Interpolation coefficients for amplitude
B[]:          Interpolation coefficients
doy:          Day Of Year
nmax:         interpolation degree (default 9)
mmax:         interpolation order (default 9)
pos:      geographic coordinate of receiver

Output:     Interpolated value at given location and DOY
******************************************************/
extern double Legendre_Interpolation(const double a_mean[], const double a_amp[], const double b_mean[], const double b_amp[],
	int nmax, int mmax, int doy, const double *pos)
{
	...
	x = cos(pos[0])*cos(pos[1]);
	y = cos(pos[0])*sin(pos[1]);
	z = sin(pos[0]);
	...
	for (n = 0; n<(nmax + 1); n++)
	{
		...
		{
			i = i + 1;
			d_mean += a_mean[i - 1] * V[n][m] + b_mean[i - 1] * W[n][m];
			d_amp += a_amp[i - 1] * V[n][m] + b_amp[i - 1] * W[n][m];
...
*			  lat		I	 latitude (rad)
*			  lon		I	 longitude (rad)
*			  hgt		I	 geodetic height from mean sea level(m)
*			  zd		I	 zenith distance (rad)
* ruturn	: gmfh		IO   hydostatic mapping function
*			  gmfw		IO   wet mapping function
* note		: added by Xiang from GMF.f in IERS
*----------------------------------------------------------------------------*/
extern void gmf_(double mjd, double lat, double lon, double hgt, double zd,
	double *gmfh, double *gmfw)
{
	const double ah_mean[55] = {
		+1.2517e2, +8.503e-01, +6.936e-02, -6.760e+00, +1.771e-1,
		+1.130e-2, +5.963e-1, +1.808e-2, +2.801e-3, -1.414e-3,
		-1.212e+0, +9.300e-2, +3.683e-3, +1.095e-3, +4.671e-5,
	...
	const double bh_mean[55] = {
		+0.000e+0, +0.000e+0, +3.249e-2, +0.000e+0, +3.324e-2,
		+1.850e-2, +0.000e+0, -1.115e-1, +2.519e-2, +4.923e-3,
		+.000    , +2.737e-2, +1.595e-2, -7.332e-4, +1.933e-4,
		+.000    , -4.796e-2, +6.381e-3, -1.599e-4, -3.685e-4,
		+1.815e-5, +0.000  , +7.033e-2, +2.426e-3, -1.111e-3,
		-1.357e-4, -7.828e-6, +2.547e-6, +0.000e+0, +5.779e-3,
		+3.133e-3, -5.312e-4, -2.028e-5, +2.323e-7, -9.100e-8,
		-1.650e-8, +0.000e+0, +3.688e-2, -8.638e-4, -8.514e-5,
		-2.828e-5, +5.403e-7, +4.390e-7, +1.350e-8, +1.800e-9,
		+0.000e+0, -2.736e-2, -2.977e-4, +8.113e-5, +2.329e-7,
		+8.451e-7, +4.490e-8, -8.100e-9, -1.500e-9, +2.000e-10
	};
	const double ah_amp[55] = {
		-2.738e-1, -2.837e+0, +1.298e-2, -3.588e-1, +2.413e-2,
		...
		+2.475e-7, -8.850e-8, -3.600e-8, -2.900e-9, +0.000e+0
	};
	const double bh_amp[55] = {
		+0.000e+0, +0.000e+0, -1.136e-1, +0.000e+0, -1.868e-1,
		-1.399e-2, +0.000e+0, -1.043e-1, +1.175e-2, -2.240e-3,
		+0.000e+0, -3.222e-2, +1.333e-2, -2.647e-3, -2.316e-5,
		+0.000e+0, +5.339e-2, +1.107e-2, -3.116e-3, -1.079e-4,
		-1.299e-5, +0.000e+0, +4.861e-3, +8.891e-3, -6.448e-4,
		-1.279e-5, +6.358e-6, -1.417e-7, +0.000e+0, +3.041e-2,
		+1.150e-3, -8.743e-4, -2.781e-5, +6.367e-7, -1.140e-8,
		-4.200e-8, +0.000e+0, -2.982e-2, -3.000e-3, +1.394e-5,
		-3.290e-5, -1.705e-7, +7.440e-8, +2.720e-8, -6.600e-9,
		+0.000e+0, +1.236e-2, -9.981e-4, -3.792e-5, -1.355e-5,
		+1.162e-6, -1.789e-7, +1.470e-8, -2.400e-9, -4.000e-10
	};
	const double aw_mean[55] = {
		+5.640e+1, +1.555e+0, -1.011e+0, -3.975e+0, +3.171e-2,
		+1.065e-1, +6.175e-1, +1.376e-1, +4.229e-2, +3.028e-3,
		+1.688e+0, -1.692e-1, +5.478e-2, +2.473e-2, +6.059e-4,
		+2.278e+0, +6.614e-3, -3.505e-4, -6.697e-3, +8.402e-4,
		+7.033e-4, -3.236e+0, +2.184e-1, -4.611e-2, -1.613e-2,
		-1.604e-3, +5.420e-5, +7.922e-5, -2.711e-1, -4.406e-1,
		-3.376e-2, -2.801e-3, -4.090e-4, -2.056e-5, +6.894e-6,
		+2.317e-6, +1.941e+0, -2.562e-1, +1.598e-2, +5.449e-3,
	...
	const double bw_mean[55] = {
		+0.000e+0, +0.000e+0, +2.592e-1, +0.000e+0, +2.974e-2,
		-5.471e-1, +0.000e+0, -5.926e-1, -1.030e-1, -1.567e-2,
		+0.000e+0, +1.710e-1, +9.025e-2, +2.689e-2, +2.243e-3,
		+0.000e+0, +3.439e-1, +2.402e-2, +5.410e-3, +1.601e-3,
		+9.669e-5, +0.000e+0, +9.502e-2, -3.063e-2, -1.055e-3,
		-1.067e-4, -1.130e-4, +2.124e-5, +0.000e+0, -3.129e-1,
		+8.463e-3, +2.253e-4, +7.413e-5, -9.376e-5, -1.606e-6,
		+2.060e-6, +0.000e+0, +2.739e-1, +1.167e-3, -2.246e-5,
		-1.287e-4, -2.438e-5, -7.561e-7, +1.158e-6, +4.950e-8,
		+0.000e+0, -1.344e-1, +5.342e-3, +3.775e-4, -6.756e-5,
		-1.686e-6, -1.184e-6, +2.768e-7, +2.730e-8, +5.700e-9
	};
	const double aw_amp[55] = {
		+1.023e-1, -2.695e+0, +3.417e-1, -1.405e-1, +3.175e-1,
		+2.116e-1, +3.536e+0, -1.505e-1, -1.660e-2, +2.967e-2,
		+3.819e-1, -1.695e-1, -7.444e-2, +7.409e-3, -6.262e-3,
		-1.836e+0, -1.759e-2, -6.256e-2, -2.371e-3, +7.947e-4,
		+1.501e-4, -8.603e-1, -1.360e-1, -3.629e-2, -3.706e-3,
		-2.976e-4, +1.857e-5, +3.021e-5, +2.248e+0, -1.178e-1,
		...
		-1.632e+0, -6.948e-3, -3.893e-3, +8.592e-4, +7.577e-5,
		+4.539e-6, -3.852e-7, -2.213e-7, -1.370e-8, +5.800e-9
	};
	const double bw_amp[55] = {
		+0.000e+0, +0.000e+0, -8.865e-2, +0.000e+0, -4.309e-1,
		+6.340e-2, +0.000e+0, +1.162e-1, +6.176e-2, -4.234e-3,
		+0.000e+0, +2.530e-1, +4.017e-2, -6.204e-3, +4.977e-3,
		+0.000e+0, -1.737e-1, -5.638e-3, +1.488e-4, +4.857e-4,
		-1.809e-4, +0.000e+0, -1.514e-1, -1.685e-2, +5.333e-3,
		-7.611e-5, +2.394e-5, +8.195e-6, +0.000e+0, +9.326e-2,
		-1.275e-2, -3.071e-4, +5.374e-5, -3.391e-5, -7.436e-6,
		+6.747e-7, +0.000e+0, -8.637e-2, -3.807e-3, -6.833e-4,
		-3.861e-5, -2.268e-5, +1.454e-6, +3.860e-7, -1.068e-7,
		+0.000e+0, -2.658e-2, -1.947e-3, +7.131e-4, -3.506e-5,
		+1.885e-7, +5.792e-7, +3.990e-8, +2.000e-8, -5.700e-9
	};

	int m, n, ir, i, j, k, count_i;
	double t, doy, dfac[20], sum1 = 0.0, p[100], ap[55], bp[55];
	double c0h, aha, c10h, c11h, ahm, awa, phh, awm, ah, bh, ch, aw, bw, cw;
	double beta, a_ht__, b_ht__, c_ht__, sine, ht_corr_coef__,
	...
	/*determine n! (factorial) moved by 0*/
	dfac[0] = 1;
	for (i = 0; i < (2 * n + 1); i++) dfac[i + 1] = dfac[i] * (i + 1);
	...
	for (i = 0; i <= n; i++){
		for (j = 0; j <= MIN(i, m); j++){
			...
			/* Legendre functions*/
			p[i + 10 * j] = 1 / pow(2, i)*sqrt(pow((1 - pow(t, 2)), j))*sum1;
		}
	}
	...
	for (i = 0; i <= n; i++){
		for (j = 0; j <= i; j++){
			ap[count_i] = p[i + 10 * j] * cos(lon*j);
			bp[count_i] = p[i + 10 * j] * sin(lon*j);
			count_i++;
		}
	}
	...
	bh = .0029;
	c0h = .062;
	if (lat < 0.) { phh = PI; c11h = .007; c10h = .002; }/* SOUTHERN HEMISPHERE */
	else { phh = 0.; c11h = .005; c10h = .001; }/* NORTHERN HEMISPHERE */


	ch = c0h + ((cos(doy / 365.25 * 2 * PI + phh) + 1.) *
		c11h / 2. + c10h) * (1. - cos(lat));
	ahm = 0.;
	aha = 0.;
	...
	*gmfh = topcon / (sine + gamma);
	...
	/*     Height correction for hydrostatic mapping function from Niell (1996) ??confusing why two gmfh*/
	a_ht__ = 2.53e-5;
	b_ht__ = .00549;
	c_ht__ = .00114;
	hs_km__ = hgt / 1e3;
