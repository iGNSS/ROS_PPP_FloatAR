...
// static int test_nmea_gq(const char *buff)
// {
...
extern int input_oem4(raw_t *raw, unsigned char data)
{
...

    {
        ...
        {
            // binNMEA = 1
            raw->nmeabuff[raw->nmeasize] = data;
            if(raw->nmeabuff[raw->nmeasize-1] == 0x0d && data == 0x0a)
            {
                binNMEA = 0;
                raw->nmeasize++;
                raw->nmeabuff[raw->nmeasize] = 0x00;
                ...
                if(raw->nmeabuff[3] == 'G')
                {
                    strcpy(raw->cstrParsedGPGGA, raw->nmeabuff);
                    // DEBUG
                    // printf("Find NMEA GPGGA: >%s<\r\n", raw->nmeabuff);
                }
                else if(raw->nmeabuff[3] == 'R')
                {
                    strcpy(raw->cstrParsedGPRMC, raw->nmeabuff);
                    // DEBUG
                    // printf("Find NMEA GPRMC: >%s<\r\n", raw->nmeabuff);
                }
